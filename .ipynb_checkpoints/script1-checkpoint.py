...
def detectAndTrackMultipleFaces():
    ...
    try:
        while True:
            ...
            for fid in faceTrackers.keys():
                ...
                tracked_position =  faceTrackers[fid].get_position()
                trackers += [[frameCounter,fid,tracked_position.left(),tracked_position.top(),tracked_position.width(),tracked_position.height()]]
                # print(type(tracked_position.left()))
                t_x = int(tracked_position.left())
                t_y = int(tracked_position.top())
                t_w = int(tracked_position.width())
                t_h = int(tracked_position.height())
                ...
                cropped_images.append(ImgArr(baseImage,t_y,t_x,t_w,t_h))
    ...
    for i in range(100):
        cv2.imshow("img_"+str(i),cropped_images[i])
        if i == 0 or i == 2 or i == 3 or i == 4 or i == 42 or i == 43 or i==44:
            cv2.imwrite("img_"+str(i)+".jpeg",cropped_images[i])
        cv2.waitKey(0)
